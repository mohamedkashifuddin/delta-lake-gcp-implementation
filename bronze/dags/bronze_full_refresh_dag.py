...
def validate_confirmation(**context):
    ...
    Full refresh DELETES all bronze data - requires explicit confirmation.
    """
    ...
    if confirm != 'YES':
        raise ValueError(
            ...
            "⚠️  WARNING: This operation will DELETE all bronze data and reload from scratch.\n\n"
            "To proceed, trigger DAG with: {\"confirm_full_refresh\": \"YES\"}\n"
            "(Type YES in all caps)"
        )
    
    # Generate batch ID and cluster name
    batch_id = f"full-refresh-{datetime.now().strftime('%Y%m%d%H%M%S')}"
    cluster_name = f"bronze-full-refresh-{datetime.now().strftime('%Y%m%d%H%M%S')}"
    ...
    print("⚠️  All bronze data will be OVERWRITTEN")
...
with models.DAG(
    dag_id="bronze_full_refresh",
    description="⚠️ DESTRUCTIVE: Reloads ALL bronze data from scratch (manual trigger, requires confirmation)",
    default_args=default_args,
    schedule_interval=None,
    start_date=days_ago(1),
    catchup=False,
    tags=["bronze", "delta-lake", "full-refresh", "manual", "destructive"],
    params={
        "confirm_full_refresh": Param(
            default="NO",
            type="string",
            title="⚠️ Confirm Full Refresh",
            description='Type "YES" (all caps) to confirm you want to DELETE all bronze data and reload from scratch.',
            enum=["NO", "YES"]
        )
    }
) as dag:
    ...
    run_full_refresh = DataprocSubmitJobOperator(
        ...
        job={
            ...
            "pyspark_job": {
                "main_python_file_uri": f"{GCS_BUCKET}/airflow/jobs/bronze_full_refresh.py",
                "args": [
                    f"{GCS_BUCKET}/raw/20241202/day*.csv",
                    "{{ ti.xcom_pull(task_ids='validate_confirmation', key='batch_id') }}"
                ]
            }
        }
    )

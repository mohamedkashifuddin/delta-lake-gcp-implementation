...
def generate_cluster_name(**context):
    cluster_name = f"gold-dim-merchant-{datetime.now().strftime('%Y%m%d%H%M%S')}"
    context['ti'].xcom_push(key='cluster_name', value=cluster_name)
    return cluster_name

with models.DAG(
    dag_id="gold_dim_merchant_scd2",
    description="Gold dim_merchant SCD Type 2 (tracks merchant name changes)",
    default_args=default_args,
    schedule_interval="0 4 * * *",  # 4 AM daily (after Silver at 3 AM)
    start_date=days_ago(1),
    catchup=False,
    tags=["gold", "delta-lake", "dimensions", "scd2", "merchant"]
) as dag:
    ...
    load_dim_merchant = DataprocSubmitJobOperator(
        task_id="load_dim_merchant_scd2",
        project_id=PROJECT_ID,
        region=REGION,
        job={
            "placement": {
            ...
            "pyspark_job": {
                "main_python_file_uri": f"{GCS_BUCKET}/airflow/jobs/gold_dim_merchant_scd2.py"
            }
        }
    )
    ...
    create_cluster >> load_dim_merchant >> delete_cluster
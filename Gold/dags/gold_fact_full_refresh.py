...
def validate_confirmation(**context):
    ...
    Full refresh DELETES all Gold fact data - requires explicit confirmation.
    """
    ...
    if confirm != 'YES':
        raise ValueError(
            "❌ Gold full refresh NOT confirmed.\n\n"
            "⚠️  WARNING: This operation will DELETE all fact_transactions data and reload from Silver.\n"
            "   Dimensions will remain unchanged.\n\n"
            "To proceed, trigger DAG with: {\"confirm_full_refresh\": \"YES\"}\n"
            "(Type YES in all caps)"
        )
    
    cluster_name = f"gold-full-refresh-{datetime.now().strftime('%Y%m%d%H%M%S')}"
    
    context['ti'].xcom_push(key='cluster_name', value=cluster_name)
    
    print("✅ Gold full refresh confirmed")
    print(f"   Cluster: {cluster_name}")
    print("⚠️  All fact_transactions data will be OVERWRITTEN")
    print("ℹ️  Dimensions will NOT be affected")
...
with models.DAG(
    dag_id="gold_fact_full_refresh",
    description="⚠️ DESTRUCTIVE: Reloads ALL fact_transactions from Silver (manual trigger, requires confirmation)",
    default_args=default_args,
    schedule_interval=None,
    start_date=days_ago(1),
    catchup=False,
    tags=["gold", "delta-lake", "fact", "full-refresh", "manual", "destructive"],
    params={
        "confirm_full_refresh": Param(
            default="NO",
            type="string",
            title="⚠️ Confirm Full Refresh",
            description='Type "YES" (all caps) to confirm you want to DELETE all fact_transactions and reload from Silver.',
            enum=["NO", "YES"]
        )
    }
) as dag:
    ...
    run_full_refresh = DataprocSubmitJobOperator(
        ...
        job={
            ...
            "pyspark_job": {
                "main_python_file_uri": f"{GCS_BUCKET}/airflow/jobs/fact_full_refresh.py"
            }
        }
    )
